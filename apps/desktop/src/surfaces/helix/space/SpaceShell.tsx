...
function SpaceEditor({
  ...
  onSuggest?: () => void;
  suggesting?: boolean;
  suggestedCollectionCount?: number;
}) {
  const { t } = useTranslation("helix");
  return (
    <div className="grid gap-5">
      <section className="grid gap-3">
        ...
        {onSuggest && (
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-signal/20 bg-signal/[0.045] px-3 py-2.5">
            <div className="min-w-0">
              <p className="text-[11px] font-medium text-fg">
                {t("helix:space.aiSuggestionTitle", "Configure o ponto de partida")}
              </p>
              <p className="mt-0.5 text-[10px] leading-relaxed text-faint">
                {suggestedCollectionCount > 0
                  ? t("helix:space.aiSuggestionApplied", {
                      count: suggestedCollectionCount,
                      defaultValue: `${suggestedCollectionCount} coleções sugeridas. Revise antes de criar.`,
                    })
                  : t(
                      "helix:space.aiSuggestionHint",
                      "O Helix sugere instruções, memória, perfil e coleções. Tudo continua editável.",
                    )}
              </p>
            </div>
            <Button
              ...
              size="sm"
              onClick={onSuggest}
              disabled={!form.name.trim() || !form.purpose.trim() || suggesting}
              className="shrink-0 gap-1.5"
            >
              <Sparkles className={`h-3.5 w-3.5 ${suggesting ? "animate-pulse" : ""}`} />
              {suggesting
                ? t("helix:space.aiSuggesting", "Sugerindo…")
                : t("helix:space.aiSuggest", "Sugerir com IA")}
            </Button>
          </div>
        )}
      </section>
...
export function SpaceShell({ ws, onBack, onOpenChat, profiles }: Props) {
  ...
  const [suggestedCollections, setSuggestedCollections] = useState<SuggestedSpaceCollection[]>([]);
  ...
  const handleSuggest = async () => {
    const suggestion = await ws.suggestSpaceConfig(form.name.trim(), form.purpose.trim(), profiles);
    if (!suggestion) return;
    updateForm({
      instructions: suggestion.instructions,
      preferredLayout: suggestion.preferredLayout,
      profileId: suggestion.profileId ?? "",
      memoryEnabled: suggestion.memoryEnabled,
    });
    setSuggestedCollections(suggestion.collections);
  };
  ...
  const handleCreate = async () => {
    ...
    const id = await ws.createSpace({
      ...
      initialCollections: suggestedCollections,
    });
    setCreating(false);
    if (!id) return;
        ...
        <div className="mb-6 overflow-hidden rounded-lg border border-line helix-view-enter">
          ...
          <div className="grid gap-6 p-5 lg:grid-cols-[minmax(0,1fr)_230px]">
            <SpaceEditor
              ...
              suggesting={ws.suggesting}
              suggestedCollectionCount={suggestedCollections.length}
              onBrowse={async () => {
...
const COLLECTION_TEMPLATES: Array<{
  id: string;
  name: string;
  fields: Array<Omit<SpaceField, "id">>;
}> = [
  {
    id: "tasks",
    name: "Tarefas",
    fields: [
      ...
      { name: "Status", type: "select", required: true, options: ["Pendente", "Em progresso", "Feita"] },
  ...
  },
  {
    ...
    fields: [
      { name: "Título", type: "text", required: true },
      { name: "Conteúdo", type: "text", required: false },
      { name: "Revisada", type: "boolean", required: false },
    ],
  },
  {
    id: "contacts",
    ...
    fields: [
      ...
      { name: "Contato", type: "text", required: false },
      { name: "Categoria", type: "select", required: false, options: ["Cliente", "Parceiro", "Outro"] },
    ],
  },
];

const FIELD_HINTS: Record<SpaceFieldType, string> = {
  text: "Texto livre — títulos, nomes e descrições",
  number: "Valores numéricos — quantidades e medidas",
  currency: "Valores monetários — preço, custo e orçamento",
  date: "Datas — prazos, eventos e marcos",
  boolean: "Sim ou não — estados e confirmações",
  select: "Lista de opções — status, categoria e prioridade",
};
...
function SpaceCollections({ ws }: { ws: Props["ws"] }) {
  ...
  const applyTemplate = (template: (typeof COLLECTION_TEMPLATES)[number]) => {
    setName(template.name);
    setFields(template.fields.map((field) => ({ ...field, id: crypto.randomUUID() })));
  };
    ...
    <div className="grid gap-5 lg:grid-cols-[280px_minmax(0,1fr)]">
      <aside className="border-r border-line pr-4">
        ...
        <div className="mt-4 border-t border-line pt-4">
          <span className="text-[9px] font-medium uppercase tracking-[0.12em] text-faint">
            {t("helix:space.collectionTemplates", "Começar com")}
          </span>
          <div className="mt-2 flex flex-wrap gap-1.5">
            {COLLECTION_TEMPLATES.map((template) => (
              <button
                ...
                className="rounded-md border border-line px-2 py-1 text-[10px] text-mute transition-colors hover:border-signal/35 hover:text-fg"
              >
                {template.name}
              </button>
            ))}
            <button
              ...
              className="rounded-md border border-line px-2 py-1 text-[10px] text-mute transition-colors hover:border-signal/35 hover:text-fg"
            >
              {t("helix:space.customCollection", "Custom")}
            </button>
          </div>
          <input
            value={name}
          ...
          <div className="mt-3 grid gap-2.5">
            {fields.map((field, index) => (
              <div key={field.id} className="rounded-lg border border-line p-2.5">
                ...
                <p className="mt-1.5 text-[9px] leading-relaxed text-faint">{FIELD_HINTS[field.type]}</p>
                <div className="mt-2 flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-0.5">
                    <button
                      ...
                      disabled={index === 0}
                      className="rounded p-1 text-faint hover:bg-white/[0.05] hover:text-fg disabled:opacity-25"
                      aria-label={t("helix:space.moveFieldUp", "Mover campo para cima")}
                    >
                    ...
                    <button
                      ...
                      className="rounded p-1 text-faint hover:bg-white/[0.05] hover:text-fg disabled:opacity-25"
                      aria-label={t("helix:space.moveFieldDown", "Mover campo para baixo")}
                    >
                    ...
                    <button
                      ...
                      className="rounded p-1 text-faint hover:bg-bad/10 hover:text-bad disabled:opacity-25"
                      aria-label={t("helix:space.removeField", "Remover campo")}
                    >
          ...
          <div className="mt-3 overflow-hidden rounded-lg border border-line">
            <div className="border-b border-line px-2.5 py-1.5 text-[9px] uppercase tracking-[0.12em] text-faint">
              {t("helix:space.collectionPreview", "Preview")}
            </div>
                ...
                <thead>
                  <tr>
                    {fields.map((field) => (
                      <th
                        key={field.id}
                        className="whitespace-nowrap border-r border-line px-2 py-2 font-medium text-mute last:border-r-0"
                      >
                        {field.name || t("helix:space.unnamedField", "Sem nome")}
                      </th>
                    ))}
                  </tr>
                </thead>
