      ...
      <div className="flex-1 overflow-y-auto px-3 pt-3 pb-3 flex flex-col gap-3">
        ...
        {p.taskActive && (
          <div className="flex flex-col gap-2.5">
            ...
            <p
              className={`text-xs leading-relaxed line-clamp-3 select-text ${p.error ? "text-bad" : p.result ? "text-fg" : "text-mute"}`}
            >
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="h-8 px-2.5 rounded-md border border-line text-[10px] font-semibold text-mute hover:text-fg transition-colors cursor-pointer"
              >
                Normal
              </button>
            </div>
          ...
          {clipboardActions.map((action) => {
            ...
            return (
              <button
                ...
                disabled={disabled}
                className="min-h-[56px] rounded-lg border border-line text-mute hover:text-fg hover:border-signal/30 transition-all cursor-pointer flex items-center gap-2 px-2.5 py-2 text-left disabled:opacity-40 disabled:pointer-events-none"
              >
          ...
          {freeActions.map((action) => {
            ...
            return (
              <button
                key={action.id}
                type="button"
                onClick={() => p.onStarterAction(action.prompt, action.executionMode)}
